...
<body class="bg-slate-50 text-slate-800 antialiased">
        ...
        <nav class="mx-auto flex w-full max-w-[1280px] items-center justify-between px-6 lg:px-8 py-4">
            ...
            <div class="hidden items-center gap-6 lg:gap-10 text-xs sm:text-[13px] font-bold tracking-widest text-white md:flex">
                ...
                <div x-data="{ open: false, timer: null }"
                    ...
                    <button type="button"
                        @click="open = !open"
                        class="flex items-center gap-1.5 uppercase font-bold tracking-widest text-white hover:text-cyan-300 transition cursor-pointer focus:outline-none"
                        :class="{ 'text-cyan-300': open }">
                        <span>INFORMASI</span>
                    ...
                    <div x-show="open"
                        ...
                        <div class="rounded-2xl bg-white p-2 shadow-2xl ring-1 ring-slate-900/10 text-slate-800 tracking-normal normal-case font-medium">
                            <a href="#informasi" @click="open = false" class="flex items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-700 hover:bg-sky-50 hover:text-blue-700 transition">
    ...
    <section class="relative z-10 px-4 sm:px-6 lg:px-8" style="margin-top: 130px !important; margin-bottom: 90px !important;">
        <div class="mx-auto w-full max-w-[1200px] min-h-[675px] lg:h-[675px] overflow-hidden rounded-3xl sm:rounded-[36px] border border-white/20 bg-slate-900 shadow-2xl shadow-slate-950/40 relative flex flex-col justify-between">
            ...
            <img
                src="{{ asset('images/kepala-sekolah.png') }}?v={{ filemtime(public_path('images/kepala-sekolah.png')) }}"
            ...
            <div class="relative z-30 bg-[#0c2f4a]/95 backdrop-blur-md border-t border-white/10 px-6 sm:px-10 lg:px-12 py-4 sm:py-5">
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 divide-y sm:divide-y-0 divide-white/5">
                    ...
                    {{-- 1. Siswa Aktif --}}
                    <div class="flex items-center gap-3 py-1" x-data="animatedCounter({{ !empty($statistics['total_students']) ? $statistics['total_students'] : 1850 }}, 2600, '+', true)">
                        ...
                        <div>
                            <p class="text-base sm:text-lg font-extrabold text-white leading-none tabular-nums" x-text="display">0+</p>
                            <p class="mt-1 text-xs font-medium text-slate-300 leading-none">Siswa Aktif</p>
                        </div>
                    </div>

                    {{-- 2. Tenaga Pendidik --}}
                    <div class="flex items-center gap-3 py-1" x-data="animatedCounter({{ !empty($statistics['total_teachers']) ? $statistics['total_teachers'] : 120 }}, 2600, '+', true)">
                    ...
                    {{-- 3. Tahun Berdiri --}}
                    <div class="flex items-center gap-3 py-1" x-data="animatedCounter({{ !empty($statistics['established_year']) ? $statistics['established_year'] : 2013 }}, 2600, '', false)">
                    ...
                    {{-- 4. Program Keahlian --}}
                    <div class="flex items-center gap-3 py-1" x-data="animatedCounter({{ !empty($statistics['total_majors']) ? $statistics['total_majors'] : 5 }}, 2600, '', true)">
                    ...
                    {{-- 5. Alumni Kerja --}}
                    <div class="flex items-center gap-3 py-1" x-data="animatedCounter({{ !empty($statistics['total_alumni']) ? $statistics['total_alumni'] : 1000 }}, 2600, '+', true)">
                        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-400/30">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
    ...
    {{-- ============ EKSTRAKURIKULER ============ --}}
    <section id="ekstrakurikuler" class="border-t border-slate-200/80 bg-[#f0f6fc] py-16 sm:py-20 px-5 lg:px-8"
        x-data="{
                activeId: 'paskibra',
                items: [
                    {
                        ...
                        iconActive: '{{ asset('icon-paskib-active.png') }}',
                        iconInactive: '{{ asset('icon-paskib-inactive.png') }}'
                    },
                    {
                        id: 'futsal',
                        name: 'Futsal',
                        subtitle: 'Kerja sama & sportivitas',
                        image: '{{ asset('futsal.png') }}',
                        desc: 'Mengasah keterampilan fisik, kelincahan teknik, strategi tim, dan menjunjung tinggi sportivitas dalam olahraga futsal.',
                        iconActive: '{{ asset('icon-futsal-active.png') }}',
                        iconInactive: '{{ asset('icon-futsal-inactive.png') }}'
                    },
                    {
                        id: 'tari',
                        name: 'Tari',
                        subtitle: 'Seni & budaya',
                        image: '{{ asset('Tari.png') }}',
                        desc: 'Wadah ekspresi seni tari tradisional dan modern untuk melestarikan kekayaan budaya serta melatih keindahan gerak.',
                        iconActive: '{{ asset('icon-tari-active.png') }}',
                        iconInactive: '{{ asset('icon-tari-inactive.png') }}'
                    },
                    {
                        id: 'pikr',
                        name: 'Pikr-r',
                        subtitle: 'Kerja sama & sportivitas',
                        image: '{{ asset('Pik-r.jpeg') }}',
                        desc: 'Pusat informasi dan konseling remaja sebaya untuk membentuk generasi muda yang cerdas, peduli, sehat, dan berencana.',
                        iconActive: '{{ asset('icon-pikr-active.png') }}',
                        iconInactive: '{{ asset('icon-pikr-inactive.png') }}'
                    }
                ],
                get current() {
                    return this.items.find(i => i.id === this.activeId) || this.items[0];
                }
            }">
        <div class="mx-auto max-w-7xl">
            {{-- Section Title & Subtitle --}}
            <div class="text-center max-w-3xl mx-auto mb-10 sm:mb-12">
                <h2 class="text-2xl sm:text-4xl font-extrabold text-[#0a4870] tracking-tight">
                    Esktrakulikuler
                </h2>
            ...
            {{-- Main Card Container --}}
            <div class="rounded-[32px] bg-white p-5 sm:p-8 lg:p-10 shadow-sm border border-slate-100">
                    ...
                    <div class="lg:col-span-4 flex flex-col gap-3 sm:gap-4">
                        <template x-for="item in items" :key="item.id">
                            <button
                                ...
                                class="w-full flex items-center gap-4 rounded-2xl p-4 sm:p-4.5 text-left transition-all duration-300 cursor-pointer"
                                ...
                                {{-- Icon Image Badge (Switch between active and inactive icon) --}}
                                <div class="h-12 w-12 rounded-xl flex items-center justify-center shrink-0 overflow-hidden transition-transform duration-300">
                                    <img
                                        :src="activeId === item.id ? item.iconActive : item.iconInactive"
                                        :alt="item.name"
                                ...
                                <div>
                                    <p
                                        class="text-base font-extrabold leading-snug transition-colors"
                                        :class="activeId === item.id ? 'text-slate-900' : 'text-slate-700'"
                                        x-text="item.name"></p>
                                    <p class="text-xs text-slate-500 font-medium mt-0.5" x-text="item.subtitle"></p>
                                </div>
                            </button>
                        </template>
                    </div>

                    {{-- Right Column: Photo Container (8 cols, W: 834.75 x H: 578.25, rounded-[28px], clip content) --}}
                    <div class="lg:col-span-8 flex justify-center">
                        <div class="relative w-full max-w-[835px] aspect-[834.75/578.25] max-h-[578.25px] rounded-[28px] overflow-hidden shadow-xl bg-slate-950 flex flex-col justify-end">

                            {{-- Dynamic Background Image --}}
                            <img
                                :src="current.image"
                                :alt="current.name"
                                class="absolute inset-0 h-full w-full object-cover object-center transition-all duration-500 transform scale-100 hover:scale-105" />

                            {{-- Dark Gradient Overlay for optimal legibility --}}
                            <div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/45 to-transparent pointer-events-none"></div>

                            {{-- Overlay Content (Title, Description, Button) --}}
                            <div class="relative z-10 p-6 sm:p-8 lg:p-10 text-white">
                                {{-- Title with dot --}}
                                <div class="flex items-center gap-2">
                                    <h3 class="text-2xl sm:text-4xl font-extrabold text-white tracking-wide drop-shadow-md" x-text="current.name"></h3>
                                    <span class="h-2.5 w-2.5 rounded-full bg-white/90 shadow-sm mt-1"></span>
                                </div>

                                {{-- Description paragraph --}}
                                <p class="mt-3 text-xs sm:text-sm text-slate-200 leading-relaxed max-w-xl drop-shadow-sm font-normal" x-text="current.desc"></p>

                                {{-- Button "Selengkapnya ->" --}}
                                <div class="mt-5 sm:mt-6">
                                    <a href="#informasi" class="inline-flex items-center gap-2.5 px-5 sm:px-6 py-2.5 sm:py-3 rounded-full bg-[#0284c7] hover:bg-[#0369a1] text-white text-xs sm:text-sm font-bold shadow-lg shadow-sky-950/30 transition-all duration-200 hover:gap-3 cursor-pointer">
    ...
    <script>
        // Animasi Counter Angka Statistik (mulai dari 0 sampai nilai target)
        function animatedCounter(target, duration = 2600, suffix = '', formatThousands = true) {
            return {
                display: '0' + suffix,
                hasAnimated: false,
                init() {
                ...
                start() {
                    ...
                    const step = (currentTime) => {
                        ...
                        this.display = this.formatNum(currentVal) + suffix;

                        if (progress < 1) {
                            requestAnimationFrame(step);
                        } else {
                            this.display = this.formatNum(endVal) + suffix;
                        }
                    };
                    requestAnimationFrame(step);
                }
